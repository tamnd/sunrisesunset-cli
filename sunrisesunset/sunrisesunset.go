...
func (c *Client) Sun(ctx context.Context, lat, lng float64, date string) (*SunTimes, error) {
	if date == "" {
		date = time.Now().UTC().Format("2006-01-02")
	}
	u := fmt.Sprintf("%s/json?lat=%f&lng=%f&formatted=0&date=%s", c.cfg.BaseURL, lat, lng, date)
