...
```
sunrisesunset <command> [arguments] [flags]
```

Run `sunrisesunset <command> --help` for the full flag list on any command. This
...
| `search <query>` | Search sunrisesunset |
...
a row here per operation you declare in `sunrisesunset/domain.go`.
