...
describes what users get. Pick a format with `-o`, or let sunrisesunset choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
sunrisesunset <command> -o table     # a rounded, color-aware grid for reading
sunrisesunset <command> -o markdown  # a GitHub pipe table to paste into docs (alias: md)
sunrisesunset <command> -o list      # one record per section, easy on the eyes
sunrisesunset <command> -o jsonl     # one JSON object per line, for piping
sunrisesunset <command> -o json      # a single JSON array
sunrisesunset <command> -o csv       # spreadsheet friendly
sunrisesunset <command> -o tsv       # tab-separated
sunrisesunset <command> -o url       # just the URL column
sunrisesunset <command> -o raw       # the underlying bytes, unformatted
```
...
```bash
sunrisesunset <command> --fields id,title,url
```
...
```bash
sunrisesunset <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
sunrisesunset <command>            # a table, because this is a terminal
sunrisesunset <command> | wc -l    # JSONL, because this is a pipe
```
